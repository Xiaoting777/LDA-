...
# 🔹 2. 配置文件路径
input_path = "/Users/chenxiaoting/Downloads/我的文件/博士学习/大论文/论文数据/txt 2025/US.csv"
output_tokens_path = "/Users/chenxiaoting/Downloads/我的文件/博士学习/大论文/论文数据/txt 2025/US_cleaned_tokens_lemm.csv"
output_meta_path = "/Users/chenxiaoting/Downloads/我的文件/博士学习/大论文/论文数据/txt 2025/US_metadata.csv"
...
raw_custom_stopwords = set([
    ...
    'population', 'percent', 'one', 'two', 'new', 'said', 'would',
    'could', 'also', 'many', 'including', 'however', 'say', 'make',
    'number', 'need', 'high', 'item', 'subject', 're', 'edu', 'use',
    'million', 'billion', 'due', 'according', 'get', 'right', 'young',
    'old', 'leave', 'should', 'data', 'based', 'day', 'photo'
])
...
try:
    with open(extra_stopwords_path, 'r', encoding='utf-8') as file:
        file_stopwords = set(line.strip() for line in file if line.strip())  # 去除空行和空格
...
# 🔹 8. 日志记录
logging.basicConfig(filename='preprocess_us.log', level=logging.INFO)
...
def process_english_csv(input_path):
    ...
    # 确保 CSV 包含需要的列
    required_columns = ['source', 'date', 'title', 'content01', 'content02']
    ...
    # 合并标题、导言和文本内容
    print("🔹 正在合并标题、导言和文本...")
    df['full_content'] = df['title'].astype(str) + ' ' + df['content01'].astype(str) + ' ' + df['content02'].astype(str)
